...
3. ユーザー入力をラッピング:
   ...
   ```
4. `CRITICAL` 制約をシステムプロンプトの**先頭**に付加（ユーザー指定のシステムプロンプトよりも前）し、タグ名を明示してモデルにデータとして扱うよう指示。
...
`--debug` を指定すると、各 HTTP 呼び出しの前後で API リクエストとレスポンスの全内容を
stderr に出力します。ブロッキング（`chatOnce`）とストリーミング（`ChatStream`）の両方が
対象です。ペイロードが有効な JSON の場合はプリティプリントされ、そうでない場合は
生バイト列として出力されます。

データアイソレーションが正しく機能していることを確認するのに便利です: ログされた
`messages` 配列で、システムプロンプトの先頭に `CRITICAL` 制約が付加されていること、
ユーザー入力が `<user_data_XXXXXX>` タグでラッピングされていることを確認できます。

`--debug` と `--quiet` は併用可能です。`--quiet` はワーニングメッセージを抑制し、
`--debug` はリクエスト・レスポンスの詳細を出力します。
